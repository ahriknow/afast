...
- **Token bucket `try_consume_token` race condition in `InMemoryStore`**: The default `try_consume_token` on `RateLimitStore` performs 5 separate async operations (`get`, `get`, `set`, `set`, `decr`), allowing concurrent requests to interleave and over-consume tokens. `InMemoryStore` now overrides this with a truly atomic implementation that acquires a single write lock for the entire refill-and-consume sequence.
- **WS/TCP "handler not found" used user error code 102**: Code `102` was in the user error range, not the reserved system range. Added `CODE_HANDLER_NOT_FOUND` (`-90013`) and used it consistently in WS and TCP transports.
- **`RateLimitKey::Connection` for WS/TCP used only client IP**: Distinct WebSocket/TCP connections from the same IP shared the same rate-limit bucket. `ConnectionContext` now uses the full peer address (`IP:port`) for per-connection rate limiting in standalone WS and TCP transports.
- **`afast-macros` doctests compilation failure**: 14 doctests in `afast-macros` used `use afast::*` but `afast` is not available as a dependency in the proc-macro crate (circular dependency). Changed from `no_run` (still compiles) to `ignore` (skipped entirely), matching the standard pattern for proc-macro crate documentation.
