...
fn generate_common_rs(services: &[Service], _is_async: bool) -> String {
    ...
    // ─── BinaryWriter ─────────────────────────────────────────────
    lines.push("/// Little-endian binary serializer for the afast wire protocol.".to_string());
    lines.push("#[derive(Debug, Default)]".to_string());
    lines.push("pub struct BinaryWriter {".to_string());
    lines.push("    buf: Vec<u8>,".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());
    lines.push("impl BinaryWriter {".to_string());
    lines.push("    pub fn new() -> Self { Self { buf: Vec::new() } }".to_string());
    lines.push("    pub fn write_u8(&mut self, n: u8) { self.buf.push(n); }".to_string());
    lines.push("    pub fn write_i8(&mut self, n: i8) { self.buf.push(n as u8); }".to_string());
    lines.push(
    ...
    lines.push("    pub fn write_str(&mut self, s: &str) {".to_string());
    lines.push("        let bytes = s.as_bytes();".to_string());
    lines.push("        self.write_u32(bytes.len() as u32);".to_string());
    lines.push("        self.buf.extend_from_slice(bytes);".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn write_bytes(&mut self, data: &[u8]) {".to_string());
    lines.push("        self.write_u32(data.len() as u32);".to_string());
    lines.push("        self.buf.extend_from_slice(data);".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn write_raw(&mut self, data: &[u8]) {".to_string());
    lines.push("        self.buf.extend_from_slice(data);".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn into_bytes(self) -> Vec<u8> { self.buf }".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());

    // ─── BinaryReader ─────────────────────────────────────────────
    lines.push("/// Little-endian binary deserializer for the afast wire protocol.".to_string());
    lines.push("pub struct BinaryReader<'a> {".to_string());
    lines.push("    data: &'a [u8],".to_string());
    lines.push("    offset: usize,".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());
    lines.push("impl<'a> BinaryReader<'a> {".to_string());
    lines.push("    pub fn new(data: &'a [u8]) -> Self { Self { data, offset: 0 } }".to_string());
    lines.push("    pub fn read_u8(&mut self) -> u8 { let v = self.data[self.offset]; self.offset += 1; v }".to_string());
    lines.push("    pub fn read_i8(&mut self) -> i8 { self.read_u8() as i8 }".to_string());
    lines.push("    pub fn read_u16(&mut self) -> u16 {".to_string());
    lines.push(
        "        let v = u16::from_le_bytes([self.data[self.offset], self.data[self.offset + 1]]);"
            .to_string(),
    );
    lines.push("        self.offset += 2; v".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn read_i16(&mut self) -> i16 { self.read_u16() as i16 }".to_string());
    lines.push("    pub fn read_u32(&mut self) -> u32 {".to_string());
    lines.push("        let v = u32::from_le_bytes([".to_string());
    lines.push("            self.data[self.offset], self.data[self.offset + 1],".to_string());
    lines.push("            self.data[self.offset + 2], self.data[self.offset + 3],".to_string());
    lines.push("        ]);".to_string());
    lines.push("        self.offset += 4; v".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn read_i32(&mut self) -> i32 { self.read_u32() as i32 }".to_string());
    lines.push("    pub fn read_u64(&mut self) -> u64 {".to_string());
    lines.push("        let lo = self.read_u32() as u64;".to_string());
    lines.push("        let hi = self.read_u32() as u64;".to_string());
    lines.push("        (hi << 32) | lo".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn read_i64(&mut self) -> i64 { self.read_u64() as i64 }".to_string());
    lines.push("    pub fn read_f32(&mut self) -> f32 {".to_string());
    lines.push("        let mut bytes = [0u8; 4];".to_string());
    lines.push(
        "        bytes.copy_from_slice(&self.data[self.offset..self.offset + 4]);".to_string(),
    );
    lines.push("        self.offset += 4;".to_string());
    lines.push("        f32::from_le_bytes(bytes)".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn read_f64(&mut self) -> f64 {".to_string());
    lines.push("        let mut bytes = [0u8; 8];".to_string());
    lines.push(
        "        bytes.copy_from_slice(&self.data[self.offset..self.offset + 8]);".to_string(),
    );
    lines.push("        self.offset += 8;".to_string());
    lines.push("        f64::from_le_bytes(bytes)".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn read_bool(&mut self) -> bool { self.read_u8() == 1 }".to_string());
    lines.push("    pub fn read_str(&mut self) -> String {".to_string());
    lines.push("        let len = self.read_u32() as usize;".to_string());
    lines.push("        let s = std::str::from_utf8(&self.data[self.offset..self.offset + len]).unwrap().to_string();".to_string());
    lines.push("        self.offset += len;".to_string());
    lines.push("        s".to_string());
    lines.push("    }".to_string());
    lines.push("    pub fn read_bytes_len(&mut self, len: usize) -> Vec<u8> {".to_string());
    lines.push("        let v = self.data[self.offset..self.offset + len].to_vec();".to_string());
    lines.push("        self.offset += len;".to_string());
    ...
    // ─── AfastError ───────────────────────────────────────────────
    lines.push("/// Error type for afast client operations.".to_string());
    lines.push("#[derive(Debug)]".to_string());
    lines.push("pub enum AfastError {".to_string());
    lines.push("    Validation { code: i64, field: String, message: String },".to_string());
    lines.push("    Network(String),".to_string());
    lines.push("    Decode(String),".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());
    lines.push("impl std::fmt::Display for AfastError {".to_string());
    lines.push(
        "    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {".to_string(),
    );
    lines.push("        match self {".to_string());
    lines.push("            AfastError::Validation { code, field, message } => {".to_string());
    lines.push(
    ...
    lines.push(
        "            AfastError::Decode(msg) => write!(f, \"Decode error: {}\", msg),".to_string(),
    );
    lines.push("        }".to_string());
    lines.push("    }".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());
    lines.push("impl std::error::Error for AfastError {}".to_string());
    lines.push("".to_string());

    // ─── AfError (server error response) ──────────────────────────
    lines.push("/// Server-side error with code and message.".to_string());
    lines.push("#[derive(Debug)]".to_string());
    lines.push("pub struct AfError {".to_string());
    lines.push("    pub code: i64,".to_string());
    lines.push("    pub message: String,".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());
    lines.push("impl std::fmt::Display for AfError {".to_string());
    lines.push(
        "    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {".to_string(),
    );
    lines.push("        write!(f, \"AfError({}): {}\", self.code, self.message)".to_string());
    lines.push("    }".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());
    lines.push("impl std::error::Error for AfError {}".to_string());
    lines.push("".to_string());
    ...
    if !all_customs.is_empty() {
        ...
        for svc in services {
            ...
            collect_custom_type_exports_rs(&svc.handlers, &[], &mut emitted, &mut lines);
        }
        lines.push("".to_string());
    }

    // ─── AfastSocket (placeholder for long-connection support) ────
    lines.push("/// Handle for a long-connection (WebSocket/TCP) session.".to_string());
    lines.push("#[derive(Debug)]".to_string());
    lines.push("pub struct AfastSocket {".to_string());
    lines.push("    pub conn_id: u32,".to_string());
    lines.push("}".to_string());
    lines.push("".to_string());
...
fn generate_request_serialize_rs(
    ...
    match ty {
        ...
        s if s.starts_with("Vec<") => {
            ...
            if inner == "u8" {
                lines.push(format!("{}    w.write_raw(_e);", indent));
            } else {
...
pub(crate) fn generate_service_rs(
    ...
    let mut lines = Vec::new();
    lines.push("// Auto-generated by afast. DO NOT EDIT.".to_string());
    lines.push("#![allow(dead_code, unused_mut)]".to_string());
    lines.push("".to_string());
    ...
    if is_async {
        ...
        lines.push("    let mut stream = stream.lock().unwrap();".to_string());
        lines.push("    let req_id: i32 = 1;".to_string());
        lines.push("    let mut frame = BinaryWriter::new();".to_string());
        lines.push("    frame.write_i32(req_id);".to_string());
        lines.push("    frame.write_u32(handler_id);".to_string());
...
impl AFast {
    ...
    pub fn generate_rs(
        ...
        mod_lines.push("".to_string());
        fs::write(dir.join("mod.rs"), mod_lines.join("\n")).map_err(|e| Error::Io {
            message: e.to_string(),
        })?;
